...
| ---------------------------- | -------------------------------------------------------------------------- | ----------------------------- | ------------------------------------------- |
| **Hard-bound C2PA**          | Signed manifest _in_ the file                                              | No — strip/re-encode drops it | **In scope** — `clean_file` / `clean_image` |
| **Soft binding**             | Imperceptible watermark _in content_ that can resolve to a remote manifest | Yes (by design)               | **Out of scope** — pixel/audio/video signal |
