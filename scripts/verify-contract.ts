...
export function ownershipManifest(content: string): string[] {
  const section = content.split("## File ownership\n", 2)[1]?.split("\n## ", 1)[0] ?? "";
  return [...section.matchAll(/^- `([^`]+)`$/gm)].map((match) => match[1]).filter((path): path is string => path !== undefined);
}

export function isOwnedPath(path: string, manifest: readonly string[]): boolean {
  return manifest.some((entry) => entry.endsWith("/**") ? path.startsWith(entry.slice(0, -2)) : path === entry);
}
...
export function verifyContract(contractId = "CONTRACT-001"): void {
  ...
  if (missing.length > 0) throw new Error(`Contract ${contractId} is missing: ${missing.join(", ")}`);

  const manifest = ownershipManifest(content);
  if (manifest.length === 0) throw new Error(`Contract ${contractId} has an empty file ownership manifest`);

  const status = execFileSync("git", ["status", "--porcelain=v1", "--untracked-files=all"], { encoding: "utf8" });
  const outOfScope = dirtyPaths(status).filter((path) => !isOwnedPath(path, manifest));
  if (outOfScope.length > 0) throw new Error(`Contract ${contractId} has dirty out-of-scope paths: ${outOfScope.join(", ")}`);
