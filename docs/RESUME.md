...
**NOT STARTED.** Specified in `docs/product/roadmap-2026H2.md`. Builds the
dashboard chat window on the streaming foundation from CONTRACT-016 and the
session mechanism from 017A — which is why 017A was inserted ahead of it, so
the chat window is built on the fixed mechanism rather than the workaround.

**Its M0 questions are already answered**, in CONTRACT-017B's M0 evidence —
read that file before assuming any of those decisions were invented.
...
**Active contract: CONTRACT-017A** — 6 of 6 milestones evidenced, generated 2026-08-11.
...
- **HEAD:** `d016de0 CONTRACT-017A: session-based conversation continuity`
- **Working tree:** 2 changed path(s) — expected while a contract is in flight, since this repository commits once per contract
- **Last touched:** `docs/contracts/CONTRACT-017A/evidence/M0-owner-confirmation.md` at 2026-08-11T05:43Z — if a session ended abruptly, work was here
- **Next action:** every milestone is evidenced; CONTRACT-017A is ready to close
