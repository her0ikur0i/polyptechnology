...
**Its M0 questions are already answered**, in CONTRACT-017B's M0 evidence: the
session id goes in a side table `conversation_provider_sessions` keyed by
`(conversation_id, provider_id)`, and each retry attempt gets its own ledger
entry rather than rewriting the first attempt's request hash.
...
directory, so it follows whichever contract is newest:
...
**Active contract: CONTRACT-017A** — 5 of 6 milestones evidenced, generated 2026-08-11.

| Milestone | Subject                                                                                    | State                               |
| --------- | ------------------------------------------------------------------------------------------ | ----------------------------------- |
| M0        | owner confirmation, recorded above                                                         | **next**                            |
| M1        | migration and the session store                                                            | done — `M1-session-store.md`        |
| M2        | resume threaded from the driver through the gateway to the CLI                             | done — `M2-resume-threaded.md`      |
| M3        | per-attempt ledger identity, retiring the mismatch failure                                 | done — `M3-per-attempt-ledger.md`   |
| M4        | retire the prompt fingerprint; fall back to replay honestly                                | done — `M4-retire-fingerprint.md`   |
| M5        | live drill proving a second turn costs less than the first, README, security review, close | done — `M5-live-drill-and-close.md` |

- **HEAD:** `5e655c6 docs: mark CONTRACT-017B closed and point the next session at CONTRACT-017A`
- **Working tree:** 24 changed path(s) — expected while a contract is in flight, since this repository commits once per contract
- **Last touched:** `docs/product/roadmap-2026H2.md` at 2026-08-11T04:16Z — if a session ended abruptly, work was here
- **Next action:** M0 — owner confirmation, recorded above
